...
mr_method_list <- function()
{
  a <- list(
    ...
    list(
      ...
      name="MR GRIP",
      PubmedID="",
      Description="Allele coding invariant version of IVw/MR-Egger",
      use_by_default=TRUE,
      heterogeneity_test=FALSE
    )
  )
...
mr_grip <- function(b_exp, b_out, se_exp, se_out, parameters)
{
  ...
  mod <- stats::lm(grip_out ~ grip_exp -1)
  smod <- summary(mod)
  b <- stats::coefficients(smod)[1,1]
  # HC3 robust estimator of std error
  h <- grip_exp2 / sum(grip_exp2)
  se <- sqrt( sum(mod$residuals^2*grip_exp2/(1-h)^2) / sum(grip_exp2)^2 )
  pval <- 2 * stats::pt(abs(b / se), length(b_exp) - 2, lower.tail = FALSE)
  
  b.wi <- sum(b_out*(b_exp^3-3*b_exp*se_exp^2)) / sum(b_exp^4-6*b_exp^2*se_exp^2+3*se_exp^4)
  se.wi <- se * b.wi/b
  pval.wi <- 2 * stats::pt(abs(b.wi / se.wi), length(b_exp) - 2, lower.tail = FALSE)
  
  return(list(b = b, se = se, pval = pval, b.wi = b.wi, se.wi = se.wi, pval.wi = pval.wi,
